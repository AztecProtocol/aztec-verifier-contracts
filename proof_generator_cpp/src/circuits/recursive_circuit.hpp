...
template <typename OuterComposer>
class RecursiveCircuit
{
    using InnerComposer = waffle::UltraComposer;

    typedef plonk::stdlib::field_t<InnerComposer> field_ct;
    ...
    static circuit_outputs create_outer_circuit(InnerComposer &inner_composer, OuterComposer &outer_composer)
    {
        ...
        typedef
            typename std::conditional<is_ultra_to_ultra, recursive_settings, ultra_to_standard_recursive_settings>::type RecursiveSettings;
        ...
        transcript::Manifest recursive_manifest = InnerComposer::create_unrolled_manifest(prover.key->num_public_inputs);

        // Verifying the ultra (inner) proof with CIRCUIT TYPES (i.e. within a standard or ultra plonk arithmetic circuit)
        stdlib::recursion::recursion_output<outer_curve> output = stdlib::recursion::verify_proof<outer_curve, RecursiveSettings>(&outer_composer, verification_key, recursive_manifest, recursive_proof);
